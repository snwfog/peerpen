...
<div class="container-fluid">
  ...
  <h2>Number of documents: <%= peer.getDocuments().size()%></h2>
  ...
  <% for(int i = 1; i < documents.size()+1; i++){ %>
    ...
    <% } %>
      <div class="span4">
        <div class="card">
          <h2 class="card-heading simple"><%= document.getDocName() %></h2>
          <div class="card-body">
            <%--<p>isus. Etiam porta sem malesuada magna mollis euismod. Donec sed odio dui. </p>--%>
            <p><a class="btn" href="/peer/<%= peer.getId() %>/document.do/<%= document.getId()%>">View document &raquo;</a></p>
          </div>
        </div>
      </div>
    <% if ((i%3) == 0){ %>
