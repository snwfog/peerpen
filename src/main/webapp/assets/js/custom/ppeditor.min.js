...
f.push(this.editor.panel.addBoxRow(c.element.attr("id")));return f};a.prototype.getType=function(){return"Remove"};return a}(g);I=function(b){function a(c,p,b,d){this.box=c;this.pageNum=p;this.toPosition=b;this.fromPosition=d;a.__super__.constructor.call(this);this.boxIds.push(this.box.element.attr("id"));null==d&&(this.fromPosition=this.box.currentPosition())}q(a,b);a.prototype.execute=function(){return this.box.setPosition(this.toPosition.left,this.toPosition.top)};a.prototype.undo=function(){return this.box.setPosition(this.fromPosition.left,
this.fromPosition.top)};a.prototype.getType=function(){return"Modify"};a.prototype.getPageNum=function(){return this.pageNum};return a}(g);v=function(b){function a(c){this.root=c;a.__super__.constructor.call(this,this.root);this._context=this.rectSize=this.downPosition=void 0}q(a,b);a.prototype.buildElement=function(){return this.element=$("<canvas></canvas>").addClass("ppedit-canvas").attr("width","920px").attr("height","1325px")};a.prototype.bindEvents=function(){var c=this;this.element.on("containerMouseDown",
function(a,b){c.downPosition={left:b.offsetX,top:b.offsetY};return c.rectSize={width:0,height:0}}).on("containerMouseMove",function(a,b,d){if(null!=c.downPosition&&null!=c.rectSize&&null!=d)return c.rectSize.width+=d.x,c.rectSize.height+=d.y,c.drawRect(c.downPosition,c.rectSize)}).on("containerMouseLeave",function(){return c.clear()}).on("containerMouseUp",function(){null!=c.downPosition&&null!=c.rectSize&&c.root.trigger("canvasRectSelect",[{topLeft:c.downPosition,size:c.rectSize}]);return c.clear()});
return this._context=this.element.get(0).getContext("2d")};a.prototype.drawRect=function(c,a){this._context.clearRect(0,0,this.element.width(),this.element.height());this._context.globalAlpha=0.2;this._context.beginPath();this._context.rect(c.left,c.top,a.width,a.height);this._context.fillStyle="blue";return this._context.fill()};a.prototype.clear=function(){this._context.clearRect(0,0,this.element.width(),this.element.height());return this.rectSize=this.downPosition=void 0};return a}(E);r=function(b){function a(c){this.root=
c;a.__super__.constructor.call(this,this.root);this.boxes={}}q(a,b);a.CLICK_TIME_INTERVAL=200;a.prototype.buildElement=function(){this.element=$("<div></div>").addClass("ppedit-box-container");this.element.append('<p class="hDotLine"></p>');return this.element.append('<p class="vDotLine"></p>')};a.prototype.bindEvents=function(){var c,a,b,d,f=this;this.element.mousedown(function(c){c.preventDefault();return f.unSelectAllBoxes()}).dblclick(function(c){c=f.getPointClicked(c);if(0===f.getSelectedBoxes().length)return f.element.trigger("addBoxRequested",
[c])}).click(function(c){f.element.trigger("unSelectBoxes");return f.element.parent().trigger("removeToolTip")});b=this.boxes;d=[];for(a in b)c=b[a],d.push(c.bindEvents());return d};a.prototype.selectBoxesInRect=function(c){var a,b=this;a={topLeft:{left:c.topLeft.left+this.element.scrollLeft(),top:c.topLeft.top+this.element.scrollTop()},size:c.size};0>a.size.width&&(a.topLeft.left-=Math.abs(a.size.width),a.size.width=Math.abs(a.size.width));0>a.size.height&&(a.topLeft.top-=Math.abs(a.size.height),
a.size.height=Math.abs(a.size.height));return this.getAllBoxes().each(function(c,d){if(M.rectContainsRect(a,b.boxBounds($(d))))return b.boxes[d.id].select()})};a.prototype.boxBounds=function(c){return{topLeft:{left:c.position().left+this.element.scrollLeft(),top:c.position().top+this.element.scrollTop()},size:{width:c.width(),height:c.height()}}};a.prototype.addBox=function(c){null==c.element&&c.buildElement();this.element.append(c.element);c.bindEvents();return this.boxes[c.element.attr("id")]=c};
a.prototype.removeBoxes=function(c){var a,b,d,f;f=[];b=0;for(d=c.length;b<d;b++)a=c[b],this.boxes[a].element.removeClass("ppedit-box-selected").removeClass("ppedit-box-focus").remove(),f.push(delete this.boxes[a]);return f};a.prototype.getBoxesFromIds=function(c){var a,b,d,f;f=[];b=0;for(d=c.length;b<d;b++)a=c[b],null!=this.boxes[a]&&f.push(this.boxes[a]);return f};a.prototype.getBoxesFromSelector=function(c){var a,b,d,f;a={};f=c.toArray();b=0;for(d=f.length;b<d;b++)c=f[b],a[c.id]=this.boxes[c.id];
return a};a.prototype.getAllBoxes=function(){return this.element.find(".ppedit-box")};a.prototype.getSelectedBoxes=function(){return this.element.find(".ppedit-box:focus, .ppedit-box-selected, .ppedit-box-focus")};a.prototype.getNotFocusedSelectedBoxes=function(){return this.element.find(".ppedit-box-selected")};a.prototype.changeBoxOpacity=function(c,a){return this.boxes[c].element.css("opacity",a)};a.prototype.unSelectAllBoxes=function(){var c,a,b,d;b=this.boxes;d=[];for(a in b)c=b[a],c.stopMoving(),
d.push(c.element.removeClass("ppedit-box-focus").blur());return d};a._rectContainsRect=function(c,a){return a.topLeft.x>=c.topLeft.x&&a.topLeft.y>=c.topLeft.y&&a.topLeft.x+a.size.width<=c.topLeft.x+c.size.width&&a.topLeft.y+a.size.height<=c.topLeft.y+c.size.height};a.prototype.getPointClicked=function(c){return{left:event.offsetX+this.element.scrollLeft(),top:event.offsetY+this.element.scrollTop()}};a.prototype.getAllHunks=function(){var c,a,b,d;b=this.boxes;d=[];for(a in b)c=b[a],d.push({id:a,html:c.element.wrap("<div></div>").parent().html()});
return d};return a}(E);C=function(b){function a(c){this.root=c;a.__super__.constructor.call(this,this.root)}q(a,b);a.prototype.buildElement=function(){return this.element=$('       <div class="ppedit-grid">            <svg width="100%" height="100%">              <defs>                <pattern id="smallGrid" width="8" height="8" patternUnits="userSpaceOnUse">                  <path d="M 8 0 L 0 0 0 8" fill="none" stroke="gray" stroke-width="0.5"/>                </pattern>                <pattern id="grid" width="80" height="80" patternUnits="userSpaceOnUse">                  <rect width="80" height="80" fill="url(#smallGrid)"/>                  <path d="M 80 0 L 0 0 0 80" fill="none" stroke="gray" stroke-width="1"/>                </pattern>              </defs>              <rect width="100%" height="100%" fill="url(#grid)" />            </svg>      </div>')};
...
a.prototype.setFontPanelPosition=function(a,b){this.element.css("left",a+"px");return this.element.css("top",b+"px")};return a}(E);x=function(b){function a(b){this.root=b;a.__super__.constructor.call(this,this.root)}q(a,b);a.prototype.buildElement=function(){return this.element=$('            <div class="left-sidebar">              <img class="icon-set undoImg" src="/assets/images/icons/OFF/glyphicons_221_unshare.png">              <img class="icon-set redoImg" src="/assets/images//icons/OFF/glyphicons_222_share.png">              <img class="icon-set gridImg" src="/assets/images/icons/OFF/glyphicons_155_show_big_thumbnails.png">              <img class="icon-set snapImg" src="/assets/images/icons/OFF/glyphicons_023_magnet.png">          </div>')};
a.prototype.bindEvents=function(){var a=this;this.element.find(".snapImg").click(function(){return $(event.target).hasClass("snapBtn-selected")?$(event.target).removeClass("snapBtn-selected"):$(event.target).addClass("snapBtn-selected")});this.element.find(".snapImg").mouseover(function(a){return $(a.target).attr("src","/assets/images/icons/ON/glyphicons_023_magnet.png")});this.element.find(".snapImg").mouseout(function(a){return $(a.target).attr("src","/assets/images/icons/OFF/glyphicons_023_magnet.png")});this.element.find(".gridImg").click(function(){return a.root.trigger("panelClickGridBtnClick")});
this.element.find(".gridImg").mouseover(function(a){return $(a.target).attr("src","/assets/images/icons/ON/glyphicons_155_show_big_thumbnails.png")});this.element.find(".gridImg").mouseout(function(a){return $(a.target).attr("src","/assets/images/icons/OFF/glyphicons_155_show_big_thumbnails.png")});this.element.find(".undoImg").click(function(){return a.root.trigger("requestUndo")});this.element.find(".undoImg").mouseover(function(a){return $(a.target).attr("src","/assets/images/icons/ON/glyphicons_221_unshare.png")});this.element.find(".undoImg").mouseout(function(a){return $(a.target).attr("src",
"/assets/images/icons/OFF/glyphicons_221_unshare.png")});this.element.find(".redoImg").click(function(){return a.root.trigger("requestRedo")});this.element.find(".redoImg").mouseover(function(a){return $(a.target).attr("src","/assets/images/icons/ON/glyphicons_222_share.png")});return this.element.find(".redoImg").mouseout(function(a){return $(a.target).attr("src","/assets/images/icons/OFF/glyphicons_222_share.png")})};return a}(E);(function(b,a,c){var d,e,g,f;d=void 0;f={"default":"cool!"};g=null;e={init:function(a){d=
b(this);b.extend(f,a||{});g=new K(d);g.buildElement();d.append(g.element);g.bindEvents();return d},doSomething:function(a){return d},destroy:function(){return d},save:function(){return g.commandManager.getUndoJSON()},allHunks:function(){return g.area.boxesContainer.getAllHunks()},clearHistory:function(){g.commandManager.clearHistory();return d},load:function(a){g.load(a.hunks);return d}};return b.fn.ppedit=function(a){return e[a]?e[a].apply(this,Array.prototype.slice.call(arguments,1)):"object"!==
typeof a&&a?b.error("Method "+a+" does not exist on jquery.ppedit"):e.init.apply(this,arguments)}})(jQuery,window,document)}).call(this);